...
import { unlockChapterEarly } from "../[slug]/actions";
...
const EARLY_UNLOCK_COST = 10;

export function ChapterCountdown({
  chapterId,
  releaseAt,
  slug,
  userPoints,
  ...
  const handleEarlyUnlock = () => {
    ...
    startTransition(async () => {
      const result = await unlockChapterEarly(chapterId, slug);
  ...
  if (!timeLeft) {
    return null; // Chapter is released
  }
  ...
  return (
    <motion.div
      ...
      className="flex w-full max-w-xs flex-col gap-3 p-3 rounded-lg border bg-muted/50"
    >
      ...
      <Button
        ...
        title={!canUnlock ? "Complete all previous chapters' lessons to unlock early" : undefined}
      >
